...
본 프로젝트는 클라이언트-서버 기반의 채팅 애플리케이션 프로토콜을 구현합니다. 사용자들이 서버를 통해 실시간으로 메시지를 주고받을 수 있으며, 다양한 통신 모드를 지원합니다. 특히, 영어 철자 검사 기능을 포함하여 사용자 경험을 향상시키는 것을 목표로 합니다.
...
클라이언트와 서버 간의 소켓 통신은 일반적으로 **TCP/IP 프로토콜**을 기반으로 이루어집니다. 통신 과정은 다음과 같습니다.
...
서버가 여러 클라이언트에게 동일한 메시지를 한 번에 보내는 것을 브로드캐스트라고 합니다. 채팅 애플리케이션에서 전체 공지나 모든 사용자가 참여하는 대화에 사용됩니다.
...
클라이언트가 다른 특정 클라이언트에게 1:1 메시지(귓속말)를 보내는 기능은 서버를 통해 중개됩니다.
...
채팅방 기능과 같이 여러 클라이언트를 그룹으로 묶어, 해당 그룹에 속한 클라이언트들끼리만 통신하는 기능입니다.
...
- **'퀴즈 정답 도전' 기능 추가**: 사용자가 출제된 퀴즈에 대한 정답을 제출할 수 있는 기능을 추가했습니다.
  - 클라이언트 메뉴에 '3. 퀴즈 정답 도전' 항목이 생성되었습니다.
  - 사용자가 정답을 입력하면 `QUIZ_ANSWER` 명령어를 통해 서버로 전송됩니다.
  - 서버는 수신한 정답을 `MSG_RECV 퀴즈정답-<ID> :<answer>` 형식으로 모든 클라이언트에게 브로드캐스트합니다.
- **클라이언트 메뉴 번호 변경**: '퀴즈 정답 도전' 기능 추가에 따라, 기존의 '1:1 채팅'은 4번으로, '영어 선생님 채팅방 입장'은 5번으로 메뉴 번호가 변경되었습니다.
- **프로토콜 및 문서 업데이트**: `README.md`와 `report.md` 파일에 새로운 기능과 프로토콜 변경사항을 반영하여 업데이트했습니다.